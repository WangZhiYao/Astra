...
def _get_watchlist_by_user_and_name(db: Session, user_id: int, name: str):
    return db.query(Watchlist).filter(Watchlist.user_id == user_id, Watchlist.name == name).first()
...
def create_watchlist(db: Session, user_id: int, watchlist: WatchlistCreate):
    ...
    if existing_watchlist:
        return OperationResult(status=OperationStatus.CONFLICT, data=existing_watchlist)
