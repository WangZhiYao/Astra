...
def _get_watchlist_by_user_and_id(db: Session, user_id: int, watchlist_id: int):
...
def get_watchlist_by_user(db: Session, user_id: int, watchlist_id: int) -> OperationResult[schemas.Watchlist]:
    db_watchlist = _get_watchlist_by_user_and_id(db, user_id, watchlist_id)
    if not db_watchlist:
...
def delete_watchlist(db: Session, user_id: int, watchlist_id: int) -> OperationResult:
    db_watchlist = _get_watchlist_by_user_and_id(db, user_id, watchlist_id)
    if not db_watchlist:
        return OperationResult(status=OperationStatus.NOT_FOUND)
